...
async def _httpx_fetch(url: str, timeout: int = 30) -> dict:
    ...
    # Wikipedia requires a bot UA with contact info
    BOT_UA = "EAGBot/1.0 (dragon2002here@gmail.com) httpx"
    BROWSER_UA = (
    ...
    # For Wikipedia, use their API for reliable plain-text extraction
    wiki_match = re.match(
        r"https?://([a-z]+)\.wikipedia\.org/wiki/(.+)", url
    )
    if wiki_match:
...
# _crawl4ai_fetch kept for reference but replaced by _httpx_fetch
...
async def fetch_url(url: str, timeout: int = 30) -> dict:
    """Fetch text content from a URL via httpx (no browser). Example: fetch_url("https://example.com")."""
